...
public class SQLCommand extends AbstractCommand
{
  ...
  public boolean isComplete(String command)
  {
    ...
    // BEGIN commented out by snowflake to avoid conflict with begin tranx
...
//        }
    ...
    try
    {
      // commit and rollback are turned off by snowflake to avoid conflict
      // with commit/rollback transaction
      if (false && command.startsWith("commit"))
      {
        session.print("commit..");
        session.getConnection().commit();
        session.println(".done.");
      }
      else if (false && command.startsWith("rollback"))
      {
  ...
  private Map tableDeclParser(String tableDecl)
  {
    ...
    while (tokenizer.hasMoreElements())
    {
      ...
      switch (state)
      {
        ...
        case 1:
        { // table seen, waiting for potential alias.
        ...
        case 2:
        { // 'AS' seen, waiting definitly for alias.
          if (",".equals(tok))
          {
        ...
        case 3:
        {  // waiting for ',' at end of 'table (as)? alias'
          if (!",".equals(tok))
          {
            // error: ',' expected.
          }
