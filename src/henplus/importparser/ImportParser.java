  ...
  throws Exception
  {
    ...
    for (; ; )
    {
      ...
      { // need to adjust buffer
        ...
        { // fieldStart is already at 0, so increase size
      ...
      while (pos < bufferEnd)
      {
        ...
        {
          colPatternPos++;
          if (colPatternPos >= colPattern.length)
          { // match!
        ...
        {
          rowPatternPos++;
          if (rowPatternPos >= rowPattern.length)
          { // match!
            if (currentColumn < _parsers.length - 1)
            {
              System.err.println(
                  "less columns than expected in row " + currentRow + ": expected " + _parsers.length + " but got " +
                  (currentColumn + 1));
            }
            if (currentColumn < _parsers.length)
            {
